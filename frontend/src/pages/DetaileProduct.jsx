...
  const [error, setError] = useState(null);
  ...
  // State to manage which set of images is currently displayed in the thumbnails
  // 'stone' or 'natural'
  const [activeImageSet, setActiveImageSet] = useState('stone');
...
  // --- NEW FUNCTION TO HANDLE IMAGE SELECTION FOR A GIVEN FINISH ---
  const handleFinishSelect = (finishType, imageSet) => {
    if (finishType !== activeImageSet) {
      // If changing the active finish set, select the first image of that set
      setSelectedImage(imageSet[0]);
      setActiveImageSet(finishType);
    } else {
      // If selecting an image within the active set
      setSelectedImage(imageSet[0]);
    }
  };
  ...
  useEffect(() => {
    if (product) {
      // Default to stone finish first, then natural finish
      if (product.stone_finish_image) {
        setSelectedImage(product.stone_finish_image);
        setActiveImageSet('stone');
      } else if (product.natural_finish_image) {
        setSelectedImage(product.natural_finish_image);
        setActiveImageSet('natural');
      }
    }
  }, [product]);

  // Debug: Log product variant images
  useEffect(() => {
    if (product) {
      console.log('🔍 Product Stone Finish Images:', {
      ...
      console.log('🔍 Product Natural Finish Images:', {
...
  // 1. Define all possible image sources
  const mainImages = [product.img1, product.img2, product.img3, product.img4, product.img5].filter(Boolean);
...
  ].filter(Boolean);
  ...
  // 2. Determine the current image set for the thumbnails
  ...
  if (activeImageSet === 'stone' && stoneFinishImages.length > 0) {
    ...
    // Default to stone if available
    currentThumbnails = stoneFinishImages.length > 0 ? stoneFinishImages : naturalFinishImages;
  }

  // 3. Define color options, filtering out options that have no images at all
  const colorOptions = [
    { name: 'Stone', key: 'stone', images: stoneFinishImages },
    { name: 'Natural', key: 'natural', images: naturalFinishImages },
  ];
...
                <img
                  src={selectedImage || product.img1 || 'placeholder-image-url'} 
                  alt={product.pname}
                  className="w-full h-96 object-cover rounded-lg mb-4"
                />
                <div className="flex gap-2 overflow-x-auto pb-2">
                  {/* Thumbnails: Show current finish images or main images */}
...
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="flex gap-3">
                  {/* Variant Buttons - Only Stone and Natural */}
                  {colorOptions.map((option, index) => (
                    option.images.length > 0 && (
                      <button
                        key={index}
                        // When clicked, switch to the correct image set and display the first image of that set
                      ...
                      >
                        {option.name} Finish ({option.images.length})
                      </button>
                    )
                  ))}
                </div>
...
              {(() => {
                  // Using the modified minimal list from the previous update
                  const overviewItems = [
...
                    { label: 'Mattress', value: product.mattress_size }, // Added from admin panel update
                    { label: 'Warranty', value: product.warranty },
                    { label: 'Delivery', value: product.dispatch_in },
                    { label: 'Delivery Condition', value: product.delivery_condition },
                    { label: 'Brand', value: product.brand },
                    { label: 'Caring', value: product.caring || '—' }, // Added from admin panel update
                  ];
