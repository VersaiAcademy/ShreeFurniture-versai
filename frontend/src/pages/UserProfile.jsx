...
const UserProfile = () => {
  ...
  const deleteItem = async (id) => {
    try {
      setLoading(true);
      const response = await axios.post(`/api/orders/${id}/cancel`, { reason: 'User requested cancellation' }, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
                    ...
                    <div className="w-auto  md:w-[50rem]">
                      ...
                      {item.status !== "dispatched" &&
                        ...
                        item.status !== "delivered" && (
                          <>
                            <Modal id={item._id} order_id={item.order_id} />
                          </>
                        )}
                      {/* end cards */}
                    </div>
